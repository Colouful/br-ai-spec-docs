...
npx @ex/ai-spec-auto@latest init --profile vue              # 默认 L3（含 OpenSpec）
npx @ex/ai-spec-auto@latest init --profile vue --level L2   # 不要 OpenSpec 时
npx @ex/ai-spec-auto@latest init --profile react            # 默认 L3
```
...
等价命令：在项目根目录执行 `npx @ex/ai-spec-auto@latest init`、`npx @ex/ai-spec-auto@latest update`、`npx @ex/ai-spec-auto@latest check`、`npx @ex/ai-spec-auto@latest uninstall`。
...
**Q: 安装后 AI 没有遵循规范？**
A: 运行 `npx @ex/ai-spec-auto@latest check` 或 `install.sh check` 确认链接有效。部分 IDE 需要重启才能识别新的规则文件。
